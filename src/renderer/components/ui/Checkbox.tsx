...
>(({ className, label, disabled, ...props }, ref) => {
  return (
    <CheckboxPrimitive.Root
      ref={ref}
      className={cn("flex items-center space-x-2 [&:active>button]:scale-80 [&[data-state=checked]>button]:bg-blue-500 [&[data-state=checked]>button]:ring-blue-500", disabled && "pointer-events-none", className)}
      {...props}
    >
      <button className="flex h-4 w-4 shrink-0 items-center space-x-2 rounded-[4px] ring-1 ring-zinc-200 transition-all duration-200 disabled:opacity-50 pointer-events-none" disabled={disabled}>
